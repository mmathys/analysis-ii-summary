...
\begin{Rechenregeln}{Stammfunktionen, Ableitungen}{}
    ...
    \hline & &\\[0.2em]
    \textbf{Common Trig. Integrals} & &\\[0.5em]
    \hline & &\\[0.2em]
    $\int_0^{2\pi} \sin(t)\cdot \cos(t) dt$ & $= \int_0^{2\pi} \sin(t) dt $ & $= \int_0^{2\pi} \cos(t) dt = 0$\\[0.5em]
    $\int \sin^2(x) dx$ & $= \frac{1}{2} (x-\sin(x) \, \cos(x))$ & \\[0.5em]
    $\int \cos^2(x) dx$ & $= \frac{1}{2} (x+\sin(x) \, \cos(x))$ & \\[0.5em]
    \end{tabular}
\end{Rechenregeln}
